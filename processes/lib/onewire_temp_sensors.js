...
function onewireTempSensors(uid, ipcon) {
    ...
    this.tempSetResolution = function (resolution, returnCallback, errorCallback) {
        ...
        oneWire.writeCommand(0, DS18x20_WRITESCRATCH, cb1, errorCallback);
        var cb1 = function() oneWire.write(0, cb2, errorCallback);  // ALARM H (unused)
        var cb2 = function() oneWire.write(0, cb3, errorCallback); // ALARM L (unused)
        var cb3 = function() oneWire.write(data, returnCallback, errorCallback); // resolution
    };

    this.tempStartConversion = function (returnCallback, errorCallback) {
        oneWire.writeCommand(deviceAddress, DS18x20_CONVERT_TEMP,
    ...
    this.tempReadScratchPad = function (deviceAddress, returnCallback, errorCallback) {
        ...
        var read_scratchdata = function(data, status) {
            ...
            if (read_count < 9) {
                oneWire.read(read_scratchbyte, errorCallback);
        ...
        oneWire.writeCommand(deviceAddress, DS18x20_READSCRATCH,
    ...
    this.getAllTemperatures = function (callback) {
        ...
        if (self.temp_sensor_devices === null) {
            self.getTempSensors(function (error, devices) {
                if (error) {
                    doError(error);
                    return;
                }
                self.temp_sensor_devices = [];
                for (var device in devices) {
                    if (device[0] == 0x10 || device[0] == 0x28) {
                        self.temp_sensor_devices.push(device);
                    }
                }
            }
        }
    ...
    // SEE BrickletOneWire.searchBusLowLevel([returnCallback][, errorCallback])
    this.getTempSensors = function (callback) {
        self.onewire.searchBusLowLevel(function())
        self.wireResetSearch(function () {
        self.onewire.searchBus(
            function(id_count, ids, status) {
            },
            function(error) {
            });
